		...
		<el-col :span="24" class="main">
			<aside :class="collapsed?'menu-collapsed':'menu-expanded'">
				...
				<el-menu :default-active="$route.path" class="el-menu-vertical-demo" @open="handleopen" @close="handleclose" @select="handleselect" unique-opened router v-show="!collapsed">
					<template v-for="(item,index) in siderBar" v-if="!item.hidden">
				...
				<ul class="el-menu el-menu-vertical-demo collapsed" v-show="collapsed" ref="menuCollapsed">
					<li v-for="(item,index) in siderBar" v-if="!item.hidden" class="el-submenu item">
	...
	mounted() {
		...
		if (user) {
			...
			if(user.manage_level_id == 1) {
				...
				this.sysName = user.manage_college_name;
			} else {
				this.sysName = 校园生活墙;
			}
		}
